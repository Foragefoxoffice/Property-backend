...
exports.createFeeTax = asyncHandler(async (req, res) => {
  ...
  const existing = await FeeTax.findOne({
    $or: [
  ...
  if (existing) {
    throw new ErrorResponse("Fee/Tax with this name already exists", 400);
  }

  // Compute next auto code
  const existingCodes = await FeeTax.find({}, { "code.en": 1 }).lean();
...
exports.updateFeeTax = asyncHandler(async (req, res) => {
  ...
  if (name_en || name_vi) {
    const duplicate = await FeeTax.findOne({
      _id: { $ne: record._id },
      $or: [
